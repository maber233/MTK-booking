...
if (is_dir($zendDir)) {
    ...
    // Step 1: Load critical base interfaces first (in dependency order)
    $criticalInterfaces = [
    ...
    // Step 3: Load all class files after interfaces are loaded
    $classCount = 0;
    foreach ($allFiles as $file) {
        try {
            $content = file_get_contents($file);
            if (strpos($content, 'class ') !== false && strpos($content, 'namespace Zend') !== false) {
                // Skip abstract classes as they might have complex dependencies
            ...
            error_log("AUTOLOADER: Error loading class: " . $e->getMessage());
        }
    }
    
    error_log("AUTOLOADER: Loaded $classCount class files");
    
    // Step 4: Load abstract classes last
    $abstractCount = 0;
    foreach ($allFiles as $file) {
        try {
            $content = file_get_contents($file);
            if (strpos($content, 'abstract class ') !== false && strpos($content, 'namespace Zend') !== false) {
                error_log("AUTOLOADER: Loading abstract class file: " . $file);
                require_once $file;
                $abstractCount++;
            }
        } catch (Exception $e) {
            error_log("AUTOLOADER: Error loading abstract class: " . $e->getMessage());
        }
    }
    
    error_log("AUTOLOADER: Loaded $abstractCount abstract class files");
    error_log("AUTOLOADER: Total Zend components loaded: " . ($interfaceCount + $classCount + $abstractCount));
    
} else {
    error_log("AUTOLOADER: Zend directory not found at: " . $zendDir);
}

error_log("AUTOLOADER: Custom Zend autoloader completed");
